...
import ServiceProviderSearchBar from "./ServiceProviderSearchBar";
...
const ServiceProvider = () => {
  ...
  const { getSnServiceProvider } = UseNetworkCalls();
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        // Fetch data
        const response = await getSnServiceProvider(params);
  ...
  return (
    <>
      <Box>
        <Box>
          <ServiceProviderSearchBar onSearch={handleSearch} />
        </Box>
        <Box>
          <TableComponent
            columns={columnsSet}
